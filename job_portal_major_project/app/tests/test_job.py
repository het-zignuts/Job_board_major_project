def test_created_job(client, auth_headers, job_payload, get_created_company):
    headers=auth_headers(role="recruiter", current_organization=get_created_company["id"])
...
def test_get_job_by_id(client, auth_headers, get_created_job):
    headers=auth_headers(role="candidate")
