...
from job_portal_major_project.app.auth.deps import *
from job_portal_major_project.app.models.user import User
from job_portal_major_project.app.core.enum import UserRole
from job_portal_major_project.app.schemas.application import ApplicationResponse
from job_portal_major_project.app.crud.application import *
from job_portal_major_project.app.crud.job import get_job_by_id
from job_portal_major_project.app.crud.company import get_company_by_id
...
def get_application_api(application_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    if is_recruiter(current_user) or is_admin(current_user): # allow only admin and recuruiter to access application
        ...
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only rcruiter or admin can retrieve application")
