...
from job_portal_major_project.app.schemas.company import *
from job_portal_major_project.app.crud.company import *
...
def update_company_api(company_id: UUID, company: CompanyUpdate, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    if not check_ownership(current_user, company_id, session):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only owner can update company")
...
def delete_company_api(company_id: UUID, current_user: User = Depends(get_current_user), session: Session = Depends(db_session_manager.get_session)):
    if not check_ownership(current_user, company_id, session):
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Only owner can delete company")
